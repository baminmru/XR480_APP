...
unsigned short xrByteStream::calcBlockCRC(long count, unsigned short crc, unsigned char* buffer)
{
    ...
    return (unsigned short)(crc ^ ((unsigned short)0xFFFF));
}
...
int xrByteStream::GetGPIOStatus(){
	if(GetSocketState() != CCESocket::CONNECTED){
		Reconect();
	}
	if(GetSocketState() != CCESocket::CONNECTED){
	...
	int l=0;
	int wcnt=1000;
	//printf("Sending GPIOStatus packet\r\n");
	this->DropData();
	l=this->Send((const  char*)buf,cnt+3);
...
bool xrByteStream::SetGPIOEX(char Mask, char Value){
	//printf("GPIO M=%X (%X) V=%X(%X)\r\n", (int) Mask,(int) (Mask & 0x3f) ,(int) Value,(int) (Value & 0x3f));
	/*
			   ...
			   if (l>13){
					...
					if (aCnt >0){
						...
						for(int i=TagBufferSize;i<TagBufferSize+aCnt;i++){
							...
							printf("r");
							short RSSI = *((short*)((unsigned char*)res +6+ 6+ 13*i));
							TagBuffer[i].RSSI =RSSI;
							printf("R");
						}
						TagBufferSize+=aCnt;
					}
				   ...
				   if (l>11){
						...
						if (aCnt >0){
							...
							for(int i=TagBufferSize;i<TagBufferSize+aCnt;i++){
								...
								//printf("r");
								unsigned char * ptr=(unsigned char*)res + 7 + 6+  31*i;
								unsigned char rr[2];
								rr[0]=*ptr;
								rr[1]=*(ptr+1);
								short RSSI=-70*16;
...
bool xrByteStream::WriteTag( char * TagID, char * NewTag, int Antenna){
	...
	while(passcnt>0){
	   ...
	   do{
		...
		}while(getpasscnt && (l>29 || l==0)); // wait for write result
