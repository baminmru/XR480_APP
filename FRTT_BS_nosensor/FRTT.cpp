...
#define VERSION "[NOSENSOR 27.03.2013]" 
...
void xrDevice::ProcessTag(const char * TagID, int Antena, bool WriteThisTag, int isNew, TagTime * TT){
	...
	/*
	// Проверка на тип метки
	if(IsCheckOFFTag(TagID)){
		// это метка останова контроля на антене?
		Status.xrAntenns[Antena-1].anCheckFilter = 0;
		Status.Changed=true;
		// block mass event on continues special lable detection
		...
		if(_stricmp(Status.xrAntenns[Antena-1].LastSpecialTag,TagID)!=0){
			strcpy(Status.xrAntenns[Antena-1].LastSpecialTag,TagID);
			SaveEvent2(Antena,TagID,"Check OFF",false);
		}
	...
	if(IsCheckONTag(TagID)){
		// это метка запуска  контроля на антене?
		Status.xrAntenns[Antena-1].anCheckFilter = 1;
		SaveCFG(Antena,Status.xrAntenns[Antena-1].FilterType, !Status.xrAntenns[Antena-1].anActive ,!Status.xrAntenns[Antena-1].anCheckFilter ,Status.xrAntenns[Antena-1].anType,Status.xrAntenns[Antena-1].StopMask) ;
		Status.Changed=true;
		// block mass event on continues special lable detection
		if(_stricmp(Status.xrAntenns[Antena-1].LastSpecialTag,TagID)!=0){
			strcpy(Status.xrAntenns[Antena-1].LastSpecialTag,TagID);
			SaveEvent2(Antena,TagID,"Check ON",false);
		}
	...
	if(IsAntennaOFFTag(TagID)){
		...
		if(_stricmp(Status.xrAntenns[Antena-1].LastSpecialTag,TagID)!=0){
			strcpy(Status.xrAntenns[Antena-1].LastSpecialTag,TagID);
			SaveEvent2(Antena,TagID,"Antenna OFF",false);
		}
	...
	if(IsAntennaONTag(TagID)){
		...
		if(_stricmp(Status.xrAntenns[Antena-1].LastSpecialTag,TagID)!=0){
			strcpy(Status.xrAntenns[Antena-1].LastSpecialTag,TagID);
			SaveEvent2(Antena,TagID,"Antenna ON",false);
		}
...
/*
		if(IsFilterChangeTag(TagID)){
			...
			Status.Changed=true;
			SaveCFG(Antena-1,Status.xrAntenns[Antena-1].FilterType, !Status.xrAntenns[Antena-1].anActive ,!Status.xrAntenns[Antena-1].anCheckFilter ,Status.xrAntenns[Antena-1].anType,Status.xrAntenns[Antena-1].StopMask) ;
			SaveEvent2(Antena,TagID,"Manual filter change",false);
...
void xrDevice::Run(){
	...
	int count=0;
	while(!xrDevice::SubstLoaded && count <32000){
		Sleep(10);
		printf("*");
		count++;
	}
		...
		if(!xrDevice::DBLocked)
		{
			...
			if(xrBS->GetSocketState()==CCESocket::CONNECTED)
			{
				...
				for (int a=1;a<=4;a++)
				{
				    ...
				    if(true) //(MotionDetectors & mdmask[a-1])==0)
					{
									...
									if(tag->OK){
										...
										if(!BadTagProcessing(tag,isLoadPoint))
										{
											...
											if(hpos>=0){ // метка уже есть в истории
												...
												db->SubstThisTag(tag->TagID, ttFind->SubstTag );
			...
			try
			{
				...
				for(int ut=0;ut<UTags->Count();ut++)
				{
					...
					if((UTags->Get(ut))->RegestredTick + 40000 < GetTickCount())
					{
					//if((UTags->Get(ut))->LastReg + 60000 < GetTickCount()){

						// останавливаемся, только если метка была замечена более 5 раз, иначе считаем это дело помехой
						if(UTags->Get(ut)->RegisterCount  >=5)
						{
							if(!(UTags->Get(ut))->EventSent)
							{
								SaveEvent2((UTags->Get(ut))->Ant ,(UTags->Get(ut))->BadTag ,"Wrong label format",0);
								StopIt=true;
								StopMask=Status.xrAntenns[(UTags->Get(ut))->Ant-1].StopMask; 
								SystemStop((UTags->Get(ut))->Ant,"","");
								StopIt=false;
								UTags->Get(ut)->EventSent=true;
							}
						}
					}
				}
			...
			if (result) {
				...
				if(!xrDevice::HALT){
					if(SendPing()){
						...
						/*if(RewriteCounter==0){
							if(!xrDevice::HALT) RcvRewriteIDX();
						}*/
						...
						if(RewriteCounter==40){
							RewriteCounter=0;
							//if(!xrDevice::HALT)db->RebuildRewriteDB();
						}
						RewriteCounter++;
...
bool xrDevice::RcvSubst(int LineNum){
...
void xrDevice::SaveCFG( int Ant /* 0..7 */, long FT, int Active, int Check, int AType, int StopMask){
	bool found;
	int cnt;
	int idx;
